...
# --- GET TEXT FROM ARTICLE ---
    ...
    if (current_source == 0): 
        ...
        for element in article_body:
            list_of_lines.append(element.text) # store text in ascii since it's going back into the browser
...
# --- GET TRANSLATIONS ---
    ...
    with open(title + ".csv", "w") as csvfile:
        ...
        for key, name in  zip(language_keys, language_names):
            ...
            native_text = pyperclip.paste()
            if (len(native_text)) > 4900:
                correct_word_count = [(native_text[i:i+4900]) for i in range(0, len(native_text), 4900)] 
